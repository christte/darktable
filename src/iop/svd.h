    ...
    double *v)    // output v matrix v[n*n]
{
  ...
  for(int i = 0; i < n; i++)
  {
    ...
    if (i < m) 
    {
      for(int k = i; k < m; k++) scale += fabs(a[k * str + i]);
      if(scale != 0.0)
      {
        for(int k = i; k < m; k++)
        {
        ...
        if (i != n - 1) 
        {
          ...
          {
            s = 0.0;
            for(int k = i; k < m; k++) s += a[k * str + i] * a[k * str + j];
            f = s / h;
            for(int k = i; k < m; k++) a[k * str + j] += f * a[k * str + i];
          }
        }
        for(int k = i; k < m; k++) a[k * str + i] = a[k * str + i] * scale;
      }
    }
    w[i] = scale * g;
    ...
    if (i < m && i != n - 1) 
    {
      for(int k = l; k < n; k++) scale += fabs(a[i * str + k]);
      if(scale != 0.0)
      {
        for(int k = l; k < n; k++)
        {
        ...
        h = f * g - s;
        a[i*str+l] = f - g;
        for(int k = l; k < n; k++) rv1[k] = a[i * str + k] / h;
        if (i != m - 1) 
        {
          for(int j = l; j < m; j++)
          {
            s = 0.0;
            for(int k = l; k < n; k++) s += a[j * str + k] * a[i * str + k];
            for(int k = l; k < n; k++) a[j * str + k] += s * rv1[k];
          }
        }
        for(int k = l; k < n; k++) a[i * str + k] = a[i * str + k] * scale;
      }
    }
  ...
  for(int i = n - 1; i >= 0; i--)
  {
    if (i < n - 1) 
    {
      if(g != 0.0)
      {
        for(int j = l; j < n; j++) v[j * n + i] = a[i * str + j] / a[i * str + l] / g;
        /* double division to avoid underflow */
        for(int j = l; j < n; j++)
        {
          s = 0.0;
          for(int k = l; k < n; k++) s += a[i * str + k] * v[k * n + j];
  ...
  for(int i = n - 1; i >= 0; i--)
  {
    ...
    g = w[i];
    if(i < n - 1)
      for(int j = l; j < n; j++) a[i * str + j] = 0.0;
    if(g != 0.0)
    {
      g = 1.0 / g;
      if (i != n - 1) 
      {
        ...
        {
          s = 0.0;
          for(int k = l; k < m; k++) s += a[k * str + i] * a[k * str + j];
          f = (s / a[i*str+i]) * g;
          for(int k = i; k < m; k++) a[k * str + j] += f * a[k * str + i];
        }
      }
      for(int j = i; j < m; j++) a[j * str + i] = a[j * str + i] * g;
    }
    else 
    {
      for(int j = i; j < m; j++) a[j * str + i] = 0.0;
    }
    ++a[i*str+i];
  }

...
#define num_gamma 6
  ...
  for(int l = 1; l <= last_level; l++) padded[l] = dt_alloc_align(64, sizeof(float) * dl(w, l) * dl(h, l));

  // allocate pyramid pointers for output
  float *output[max_levels] = {0};
  for(int l = 0; l <= last_level; l++) output[l] = dt_alloc_align(64, sizeof(float) * dl(w, l) * dl(h, l));
  ...
  for(int k = 0; k < num_gamma; k++)
    for(int l = 0; l <= last_level; l++) buf[k][l] = dt_alloc_align(64, sizeof(float) * dl(w, l) * dl(h, l));

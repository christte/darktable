...
static int _print_job_run(dt_job_t *job)
{
  ...
  const dt_colorspaces_color_profile_t *buf_profile
      = dt_colorspaces_get_output_profile(params->imgid, params->buf_icc_type, params->buf_icc_profile);

  dt_imageio_export_with_flags(params->imgid, "unused", &buf, (dt_imageio_module_data_t *)&dat, TRUE, FALSE,
                               high_quality, upscale, FALSE, NULL, FALSE, params->buf_icc_type,
                               params->buf_icc_profile, params->buf_icc_intent, NULL, NULL, 1, 1);

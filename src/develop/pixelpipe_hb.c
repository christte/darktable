                                       ...
                                       float *pick_color_lab_max, float *pick_color_lab_mean)
{
  ...
  for(int k = 0; k < 4; k += 2) box[k] = MIN(roi_in->width - 1, MAX(0, pick_box[k] * roi_in->width));
  for(int k = 1; k < 4; k += 2) box[k] = MIN(roi_in->height - 1, MAX(0, pick_box[k] * roi_in->height));
  ...
  if(pick_size == DT_COLORPICKER_SIZE_BOX)
  {
    for(int j = box[1]; j <= box[3]; j++)
      for(int i = box[0]; i <= box[2]; i++)
      {
        for(int k = 0; k < 3; k++)
        {
          picked_color_rgb_min[k] = MIN(picked_color_rgb_min[k], pixel[4 * (roi_in->width * j + i) + k]);
          picked_color_rgb_max[k] = MAX(picked_color_rgb_max[k], pixel[4 * (roi_in->width * j + i) + k]);
  ...
  {
    for(int i = 0; i < 3; i++)
      picked_color_rgb_mean[i] = picked_color_rgb_min[i] = picked_color_rgb_max[i]
          = pixel[4 * (roi_in->width * point[1] + point[0]) + i];
  }
...
static void _pixelpipe_get_histogram_profile_type(dt_colorspaces_color_profile_type_t *out_type,
                                                  gchar **out_filename)
{
...
static void _pixelpipe_pick_live_samples(const float *const input, const dt_iop_roi_t *roi_in)
{
  ...
  const dt_colorspaces_color_profile_t *d_profile = dt_colorspaces_get_profile(
  ...
  {
    const dt_colorspaces_color_profile_t *d_histogram = dt_colorspaces_get_profile(
        histogram_type, histogram_filename, DT_PROFILE_DIRECTION_OUT | DT_PROFILE_DIRECTION_DISPLAY);
  ...
  if(display_profile && lab_profile)
    xform_rgb2lab
        = cmsCreateTransform(display_profile, TYPE_RGB_FLT, lab_profile, TYPE_Lab_FLT, INTENT_PERCEPTUAL, 0);

  // display rgb --> histogram rgb
  if(display_profile && histogram_profile)
    xform_rgb2rgb
        = cmsCreateTransform(display_profile, TYPE_RGB_FLT, histogram_profile, TYPE_RGB_FLT, INTENT_PERCEPTUAL, 0);
  ...
  while(samples)
  {
    ...
    _pixelpipe_pick_from_image(input, roi_in, xform_rgb2lab, xform_rgb2rgb, sample->box, sample->point,
                               sample->size, sample->picked_color_rgb_min, sample->picked_color_rgb_max,
                               sample->picked_color_rgb_mean, sample->picked_color_lab_min,
                               sample->picked_color_lab_max, sample->picked_color_lab_mean);
                                                ...
                                                const dt_iop_roi_t *roi_in)
{
  ...
  _pixelpipe_pick_from_image(
      input, roi_in, xform_rgb2lab, xform_rgb2rgb, dev->gui_module->color_picker_box,
      dev->gui_module->color_picker_point, darktable.lib->proxy.colorpicker.size,
      ...
      darktable.lib->proxy.colorpicker.picked_color_lab_min, darktable.lib->proxy.colorpicker.picked_color_lab_max,
...
static void _pixelpipe_final_histogram(dt_develop_t *dev, const float *const input, const dt_iop_roi_t *roi_in)
{
  ...
  dt_histogram_roi_t histogram_roi = {
    .width = roi_in->width, .height = roi_in->height, .crop_x = 0, .crop_y = 0, .crop_width = 0, .crop_height = 0
  };
     ...
     && darktable.lib->proxy.colorpicker.restrict_histogram)
  {
    if(darktable.lib->proxy.colorpicker.size == DT_COLORPICKER_SIZE_BOX)
    {
      ...
      histogram_roi.crop_width
          = roi_in->width - MIN(roi_in->width, MAX(0, dev->gui_module->color_picker_box[2] * roi_in->width));
      histogram_roi.crop_height
          = roi_in->height - MIN(roi_in->height, MAX(0, dev->gui_module->color_picker_box[3] * roi_in->height));
    }
    else
    {
      ...
      histogram_roi.crop_width
          = roi_in->width - MIN(roi_in->width, MAX(0, dev->gui_module->color_picker_point[0] * roi_in->width));
      histogram_roi.crop_height
          = roi_in->height - MIN(roi_in->height, MAX(0, dev->gui_module->color_picker_point[1] * roi_in->height));
    }
  }
         ...
         && strcmp(histogram_filename, darktable.color_profiles->display_filename)))
  {
    ...
    // display rgb --> histogram rgb
    if(display_profile && histogram_profile)
      xform_rgb2rgb = cmsCreateTransform(display_profile, TYPE_RGBA_FLT, histogram_profile, TYPE_RGBA_FLT,
                                         INTENT_PERCEPTUAL, 0);
...
static void _pixelpipe_final_histogram_waveform(dt_develop_t *dev, const float *const input,
                                                const dt_iop_roi_t *roi_in)
{
  uint32_t *buf
      = (uint32_t *)calloc(dev->histogram_waveform_height * dev->histogram_waveform_width * 3, sizeof(uint32_t));
...
float *dt_dev_get_raster_mask(const dt_dev_pixelpipe_t *pipe, const dt_iop_module_t *raster_mask_source,
                              const int raster_mask_id, const dt_iop_module_t *target_module, gboolean *free_mask)
{
  if(!raster_mask_source) return NULL;
  ...
  *free_mask = FALSE;
  ...
  for(source_iter = pipe->nodes; source_iter; source_iter = g_list_next(source_iter))
  {
    ...
    if(candidate->module == raster_mask_source) break;
  }
                  ...
                  && module->module->dev->gui_module->operation_tags_filter() & module->module->operation_tags()))
          {
            if(module->module->distort_mask
               && !(!strcmp(module->module->op, "finalscale") // hack against pipes not using finalscale
                    && module->processed_roi_in.width == 0 && module->processed_roi_in.height == 0))
            {
              float *transformed_mask = dt_alloc_align(64, sizeof(float) * module->processed_roi_out.width
              ...
              *free_mask = TRUE;
              ...
              printf("FIXME: module `%s' changed the roi from %d x %d @ %d / %d to %d x %d | %d / %d but doesn't "
                     "have "
                     "distort_mask() implemented!\n",
                     module->module->op, module->processed_roi_in.width, module->processed_roi_in.height,
                     module->processed_roi_in.x, module->processed_roi_in.y, module->processed_roi_out.width,
                     module->processed_roi_out.height, module->processed_roi_out.x, module->processed_roi_out.y);
          }
